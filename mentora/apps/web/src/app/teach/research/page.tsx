...
import { Globe, BookOpen, Copy, CheckCheck, ExternalLink, Sparkles, AlertCircle } from 'lucide-react';
...
function SummaryCard({ summary }: { summary: string }) {
  return (
    <Card padding="lg" className="bg-brand-50 border-brand-200">
      ...
      <p className="text-stone-800 leading-relaxed text-lg">{summary}</p>
    </Card>
  );
}
...
function KeyPointsCard({ points }: { points: string[] }) {
  return (
    <Card padding="lg">
      <h2 className="text-xl font-bold text-stone-900 mb-4">Key points</h2>
      <ul className="space-y-3" role="list">
        {points.map((point, i) => (
          <li key={i} className="flex gap-3 items-start">
            ...
            <span className="text-stone-800 leading-relaxed">{point}</span>
          </li>
        ))}
      </ul>
    </Card>
...
function LessonOutlineCard({
  ...
  return (
    <Card padding="lg">
      <h2 className="text-xl font-bold text-stone-900 mb-5">Suggested lesson plan</h2>
      <div className="space-y-5">
        {sections.map((section, si) => (
          ...
          >
            <h3 className="font-bold text-stone-900 mb-3 text-base">
              <span
            ...
            <ul className="space-y-2 pl-9" role="list">
              {section.points.map((point, pi) => (
                <li key={pi} className="text-stone-700 leading-relaxed text-sm flex gap-2 items-start">
                  <span className="mt-1.5 w-1.5 h-1.5 rounded-full bg-teal-400 shrink-0" aria-hidden="true" />
                  {point}
                </li>
              ))}
            </ul>
    ...
    <Card padding="lg">
      <div className="flex items-center justify-between flex-wrap gap-3 mb-5">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <span className="text-xs text-stone-400 font-medium">via {provider}</span>
        </div>
      </div>

      {sources.length === 0 ? (
        <p className="text-stone-500 italic">No sources available for this briefing.</p>
      ) : (
        <ul className="space-y-4" role="list">
          {sources.map((source, i) => (
            <li key={i} className="rounded-xl border border-surface-200 p-4 hover:border-brand-200 transition-colors">
              <a
                href={source.url}
                target="_blank"
              ...
              >
                {source.title}
                <ExternalLink size={14} aria-hidden="true" className="shrink-0 text-brand-400" />
              </a>
              {source.siteName && (
                <p className="text-xs text-stone-400 mb-1.5">{source.siteName}</p>
              )}
              <p className="text-sm text-stone-700 leading-relaxed">{source.snippet}</p>
            </li>
          ))}
        </ul>
      )}
  ...
  return (
    <div>
      ...
      <div className="mb-8 flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-stone-900 mb-2">{briefing.topic}</h1>
          <p className="text-lg text-stone-500">Here is your teacher-ready briefing.</p>
        </div>
  ...
  return (
    <div className="section">
      <div className="page-container max-w-3xl">
        {briefing ? (
          <BriefingResult briefing={briefing} onReset={() => { setBriefing(null); setTopic(''); }} />
        ) : (
          <>
            {/* Page heading */}
            <div className="mb-10">
              <div className="flex items-center gap-3 mb-3">
                ...
                <h1 className="text-stone-900">Research a topic</h1>
              </div>
              ...
              <form onSubmit={handleSubmit} noValidate>
                <div className="space-y-6">
                  ...
                  <Button
                    ...
                    icon={<Globe size={20} />}
                  >
                    {loading ? 'Searching the web and writing your briefing…' : 'Research this topic'}
                  </Button>
                </div>
              </form>
            </Card>
            ...
            {loading && (
              ...
              >
                ...
                <p className="text-stone-400">This usually takes 10–20 seconds.</p>
              </div>
            )}
